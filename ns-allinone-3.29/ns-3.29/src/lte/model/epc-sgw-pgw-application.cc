...
// For DC insert
Ipv4Address
EpcSgwPgwApplication::UeInfo::GetSenbAddr ()
{
  return m_senbAddr;
}

void
EpcSgwPgwApplication::UeInfo::SetSenbAddr (Ipv4Address senbAddr)
{
  m_senbAddr = senbAddr;
}
...
//For DC insert
void
DpcSgwPgwApplication::SetSplitAlgorithm (uint16_t splitAlgorithm)
{
...
//For DC insert
void
EpcSgwPgwApplication::RecvAssistInfo (LteRrcSqp::AssistInfo assistInfo)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_isAssistInfoSink == true, "Not a assist info sink");

  int nodeNum;
  if (assistInfo.is_enb && assistInfo.is_memb) 
    nodeNum = 0;
  else if (assistInfo.is_enb) 
    nodeNum = 1;
  else
    nodeNum = 2;
  //** not finish for following line **
  //NS_LOG_UNCOND("nodeNum " << nodeNum << " pdcp_sn " << assistInfo.pdcp_sn << " pdcp_delay " << assistInfo.pdcp_delay << " rlc_avg_buffer ")
...
EpcSgwPgwApplication::SendToS1uSocket (Ptr<Packet> packet, Ipv4Address enbAddr, uint32_t teid)
{
  ...
  //modify dc
  gtpu.SetSequenceMumber(gipu_SN);
  gtpu_SN++;
...
EpcSgwPgwApplication::DoCreateSessionRequest (EpcS11SapSgw::CreateSessionRequestMessage req)
{
  ...
  for (std::list<EpcS11SapSgw::BearerContextToBeCreated>::iterator bit = req.bearerContextsToBeCreated.begin ();
       ...
       ++bit)
    {
      ...
      //modify for dc
      bearerContext.dcType = bit->dcType;
